...
Если macOS блокирует приложение с сообщением:

> «Приложение не может быть открыто, так как его разработчик не может быть проверен»
...
2. Пролистайте вниз до раздела безопасности
...
После первого запуска приложение будет открываться нормально.
...
Ридер поддерживает:
- локальные квесты
- удалённые квесты (Remote/API)

### 1. Локальные квесты через папку в Unity-проекте

Поместите папку квеста в:

`Assets/StreamingAssets/Quests/`


### 2. Локальные квесты через интерфейс приложения
...
Позволяет добавлять локальные квесты без перезапуска приложения.


### 3. Remote-квесты

Удалённые квесты публикуются через [**Text Quest Editor**](https://github.com/albruevich/Text-Quest-Editor).

После публикации квест автоматически становится доступен в разделе **Remote** внутри **Text Quest Reader**.

---

## Важно для локальных квестов
...
- просмотр списка remote-квестов  
- загрузка без ручного копирования файлов  
- запуск квестов напрямую с сервера
